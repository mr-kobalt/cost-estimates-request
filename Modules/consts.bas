...
Public Enum PurchaseColumns
    ...
    INDEX_1C = 2 ' номер позиции в заказе 1С
    MANUFACTURER = 3 ' производитель
    PN = 4 ' артикул/продуктовый номер/партномер
    NAME_AND_DESCRIPTION = 5 ' наименование/описание
    qty = 6 ' количество
    Unit = 7 ' единица измерения
    PRICE_GPL_RECALCULATED = 8 ' цена прайс-листа после пересчёта в валюта_расчёта
    TOTAL_GPL_RECALCULATED = 9 ' сумма прайс-листа после пересчёта в валюта_расчёта
    DISCOUNT = 10 ' скидка, вычисляемая из суммы прайс-листа и суммы входа в валюте расчёта
    PRICE_PURCHASE_RECALCULATED = 11 ' цена закупки после пересчёта в валюта_расчёта
    TOTAL_PURCHASE_RECALCULATED = 12 ' сумма закупки после пересчёта в валюта_расчёта
    VAT_PURCHASE = 13 ' НДС расчёта
    DELIVERY_TIME = 14 ' срок доставки
    SUPPLIER = 15 ' поставщик
    USER_COMMENTS = 16 ' комментарии
    UNIT_WEIGHT = 17 ' вес штуки
    TOTAL_WEIGHT = 18 ' вес суммарный
    UNIT_VOLUME = 19 ' объём штуки
    TOTAL_VOLUME = 20 ' объём суммарный
    GPL_CURRENCY = 21 ' валюта прайс-листа
    PRICE_GPL = 22 ' цена прайс-листа
    PURCHASE_CURRENCY = 23 ' валюта прайс-листа
    PRICE_PURCHASE = 24 ' цена закупки
    vat = 25 ' Value Added Tax - НДС
    TOTAL_GPL = 26 ' сумма прайс-листа
    TOTAL_PURCHASE = 27 ' сумма закупки
    INDEX_DESC = 28 ' описание типа индекса
    VAT_PURCHASE_AMOUNT = 29 ' размер НДС закупки
    [_LAST]
End Enum
...
Public Const TEXTS_SUBJECT_VAT = "в т.ч. НДС 20%"
